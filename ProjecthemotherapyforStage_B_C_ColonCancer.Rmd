...
The frequency distribution of mortalityAt5years is provided below. Notice that 57.71% of the patients are alive after 5 years. 
...
$Probability(mortalityAt5years) = f( constant + \beta_1 * X_1 + \epsilon)$
...
The Extent of local spread, and less significantly the age group and "more than 4 positive lymph nodes" seem to be the main predictors of the 5 year life status. 
...
The deviance of this model is 664, lower than the NULL deviance.
Its AIC (678.64) is also lower than the NULL deviance.
...
Adding all input as predictors will give the best deviance, but it will be overfitting. By only including the 3 significant predictors (as determined by their p-value), the deviance but the AIC decreases. The AIC is a measure adjusted to penalize the inclusion of a variable that doesnt benefit the model. 
So we can conclude that the extent of the spread, the age group and 'more than 4 positive lymph nodes' are the main predictor of the life status at 5 years.
...
Here I use ANOVA to check if the chosen model deviance is significantly different from the deviance of the NULL model.

The ANOVA test indicates that it is significantly different ( p = 0.00000000000000022 < 0.05)
...
*with less than 4 positive lymphe nodes
...
**The "NULL" model and Null deviance using training set**
...
**The full model and its deviance using training set**
...
## Predicting using the training model on the testing dataset
...
The extent of the spread, the age group and the number of nymph nodes are the main predictor of a patient life status at 5 years.
The treatment is not a predictor.

Even so, the model can not be used reliably to determine the 5 year life status of a patient; its hit-ratio doesnt exceed 67%.
