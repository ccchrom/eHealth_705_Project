...
colon.survival <-
  survfit(Surv(time, status) ~ 1,  data = colon.data)
summary(colon.survival)
```
## plot
```{r,  fig.width=5, fig.height=5}
plot(colon.survival,  xlab="Days till death", ylab="Survival probability")
title("Survival curves for colon data, Null model")
```

# Survival model on RX

```{r,  fig.width=5, fig.height=5}
colon.survival <- survfit(Surv(time,status) ~ rx, data = colon.data)
summary(colon.survival)
```
## plot
```{r,  fig.width=5, fig.height=5}
plot(colon.survival, lty = c(2,1), xlab="Days till death", ylab="Survival Probability")
legend(13, .9, c("Probability of survival: Lev+5FU"), lty = 2 ) 
legend(2, .2, c("Probability of survival: Lev"), lty = 1) 
title("Survival curves for migraine data")
```
