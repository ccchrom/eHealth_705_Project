...
```  
# Abstract
  The [Canadian Cancer Society](https://www.cancer.ca/en/cancer-information/cancer-type/colorectal/statistics/?region=on) state that "Colorectal cancer is expected to be the third most commonly diagnosed cancer in Canada in 2020 (excluding non-melanoma skin cancers). It is the second leading cause of death from cancer in men, and the third leading cause of death from cancer in women in Canada.".  We aim to use the data from Moertel et. al (1990) do assess the factors that impact mortality in those with Type B/C Colon Cancer, as well as comparing the treatment of [Levimasole](https://go.drugbank.com/drugs/DB00848) against adjuvant therapy with Levimasole and [Fluorouracil](https://go.drugbank.com/drugs/DB00544).  Regressions were done - both stepwise and logistic - against two indicators of mortality - living or deceased at the 5 year mark (mort_5yr), and time until death in years (time.years).  Survival analysis was also performed on the data.

From our analysis, it was noted that the adjuvant therapy of Levimasole and Fluorouracil had longer surival rates than just Levimasole alone.  Our regression analyses did however, point out that tumor descriptors, such as local extend of the spread, and having more than four lymph nodes effects, had more of an impact on life expectancy than treatment.
...
<div class = "blue">
There are two main objectives we are trying to achieve:  

1.	To find which tumor descriptors and demographics have the greatest impact on life expectancy.  
2.	To confirm using our own process that adjuvant therapy with the combination of Levamisole and Fluorouracil increases probability of survival compared with no adjuvant therapy – using only Levamisole.


We aim to achieve this by:<br>

1.  Cleaning the data from the original and filtering it to our testing needs
2.  Using distribution plots and tables to assess the distributions of variables.  
3.	Using a logistic regression model to investigate the association between five year life status and one or more categorical predictor variables.  
4.  Using stepwise (linear) regression to obtain a model that best describes the impact of specific variables on time until death.  
5.	Assessing whether there are differences in survival (or cumulative incidence of event) among the two treatment groups.    
6.  Using correlation and Variance Inflaction Factors (VIF) to investigate if any of the tumor descriptors or demographics have an impact on each other.   
7.	Use relative importance testing to assess which of the tumor descriptors or demographics have the greatest impact on mortality.  
8.  Testing our constructed regression models on a split test sample to assess their quality.  
</div>
...
The description of the variables for this dataset are listed below.  It should be noted that they are also listed as factors.
...
<div class = "blue">
Below, character variables are converted to factors, integers are converted to numbers, and the observation arm is filtered out.  This is done to assist with our analysis.  The removal of the observation group done to primarily focus on the comparison between adjuvant therapy and Levimasole, as well as to also focus on other factors - aside from treatment - that may effect survival within that group.
</div>
```{r}
library(dplyr)
...
The total of NAs is 79 - which is roughly 12% of the filtered sample of 614.  We chose to omit these values due to the large sample size.  There was worry that if we imputed this data - which is mostly two-level factors - that there may be a large margin of error and it may skew our model.
</div>
```{r}
library(dplyr)
...
The following table gives a quick glance at the cleaned data
```{r}
...
* Majority of the study population is from +60 age group followed by 40-59 years and a very small proportion belong to <40 years age group. There is approximately equal distribution of the population in the 3 age group categories for both the treatments.  
</div>
```{r echo=FALSE, , fig.align="center", fig.width=6, fig.height=6, fig.cap="Figure 2: Tree Diagram of Cleaned Colon Dataset"}
library(vtree)
vtree(colon.data, c("rx.factor", "sex.factor", "age.factor"), horiz = FALSE, fillcolor = c(restecg = "#e7d4e8", cp = "#99d8c9"), keep = list(restecg = c("ST-T abnormality")))
```
Some preliminary observations for the variables are plotted to understand the demographic distribution, treatment distribution, time to treatment and the tumor descriptors.
...
p15 <- ggplot(plotdata10, 
  ...
  geom_text(aes(label = pctlabel), 
            vjust = -0.25) +
  scale_y_continuous(labels = percent) +
  labs(x = "Time from Surgery to Treatment", 
       y = "Percent", 
       title  = "Surgery to Treatment")
...
p01 <- ggplot(plotdata, 
  ...
  labs(x = "Perforation", 
       y = "Percent", 
       title  = "Perforation of Colon Distribution")
...
p02 <- ggplot(plotdata1, 
  ...
  labs(x = "Obstruction", 
       y = "Percent", 
       title  = "Obstruction of Colon Distribution")
...
p03 <- ggplot(plotdata2, 
  ...
  labs(x = "Adherence", 
       y = "Percent", 
       title  = "Adherence to Organs Distribution")
...
p04 <- ggplot(plotdata3, 
  ...
  labs(x = "differentiation", 
       y = "Percent", 
       title  = "Tumor Differentiation Distribution")
...
p05 <- ggplot(plotdata4, 
  ...
  labs(x = "differentiation", 
       y = "Percent", 
       title  = "Extent of Tumor Spread Distribution")
...
p07 <- ggplot(plotdata6, 
  ...
  labs(x = "4 Nodes Affected", 
       y = "Percent", 
       title  = "4 Lymph Nodes Distribution")
...
p08 <- ggplot(colon.data, aes(x=time.years)) + 
  ...
  labs(x = "time in years", 
       y = "Density", 
       title  = "Time Until Death in Years")
...
<div class = "blue">
The cleaned, recoded sample has total of 551 observations.  The preliminary findings based on the graphs above and the summary below are as follows:  
...
The odds will increase to  $e^{-2.5156 + 3.2343}$ = `r exp(-2.5156 + 3.2343)` if the extent of the spread is submucosa., but will decrease further to $e^{-2.5156 -1.3925}$ = `r exp(-2.5156 +-1.3925)` if the patient has more than four positive lymph nodes.
</div>
#### McFadden $R^2$
<div class = "blue">
...
<div class = "blue">
Every selected predictor appears significant (p-value < 0.05), and this is confirmed with no odd ratio confidence interval including 1.
</div> 
```{r}
tab_model(
  training_model,
...
0 means 'Died' and 1 means 'Alive' at 5 years. 59.85% of the test sample patients are alive after the 5 years.
...
<div class = "blue">
The data below displays both the 'actual' life status of each test sample patient against the 'predicted' of the model. A predicted value below 0.5 estimates that the patient has died whereas a value above 0.5 estimates that this person is alive at the 5 year mark. If we extrapolate the first 20 rows to the rest of the sample, the model is probably not very accurate; this is consistent with the hit-ratio calculated above.
</div>
...
A stepwise regression is used to analyze the impact that treatment and other factors have on life expectancy.  We use the same independent factors as the logistic regression did.  Using stepwise (or multiple linear) regression requires that the dependent variable is numeric - which is why we are using 'time.years' instead of the factor mort-5yr.  This is done to compare the results and add or remove validity from the logistic regression.  Moreover, it helps in determining the overall fit of the model and the relative contribution (or strength) of each of the independent variable in explaining the variance. 
...
i.e., significant relationship between each independent variable and the dependent variable.
</p>
## The Full Model
Below is the model on the full sample using all tumor descriptors and demographics.
...
```
<div class = "blue">
Using the stepAIC() function, it appears that the treatment, age, differentiation, extent of spread, and location of primary tumor have the greatest impact when assessing AIC.
</div>
...
<div class = "blue">
The RegModel.2 model above (model to the right), obtained through the stepwise regression (or stepwise selection) has the variables 'rx' (if we consider a type-1 error of 10%),'extent','loccomp' and 'node4' have coefficients that are significantly different (p<0.05) from zero.  Hence, the null hypotheses, which assume no relationships between the variables and the dependent variable ('time.years'), for these variables would be rejected. 

Moreover, The adjusted R2(0.138), coefficient of multiple determination, is slightly higher in the original model.  The AIC is smaller at 2471 - showing that it has a better fit.
...
```
<div class = "blue">
Within the training sample, it is noted that treatment should not be included as it's high p-value.  We wanted to try and leave it in initially with it being only a 10% type one error.
</div>
...
```
<div class ="blue">
It is noted once again that'node4' has the highest lmg value, followed by the 'extent', then 'loccomp' when assessing their impact on 'time.years'. 
</div>
## Reporting The Training Residuals from the Training Sample
...
## Reporting The Testing Residuals from the Training Model
Below is a table that shows the actual value of time.years for the sample, followed by the predicted value based on our model, then the residuals - or difference between them.
```{r}
...
### Testing the Distribution of Residuals using Shapiro-Wilk’s method
...
```
<div class ="blue">
From the Shapiro-Wilk normality test, we have got the p value of 0.04357. Assuming an alpha of 0.05, since the p-value is less than 0.05, null hypothesis (that distribution of residuals is normal) is  rejected. Hence it may be concluded that the residuals are ___not normally distributed___

Hence it can be concluded that the regression model (model.tr) may not be the most accurate at explaining/predicting the trends in time until death in years.
</div>
...
The p-value is the result of the chi-squared test and the null hypothesis is rejected for p-value < 0.05. We should conclude that variance around regression line is not constant throughout its range.
</div>
...
```
<div class = "blue">
It may be concluded that the model has a weak Goodness of fit since most of the points are foggy or dispersed (away from the diagonal line).

From this scatter plot, it can also be inferred that the residuals are heteroscedastic. This means that the variance of the error is not constant across various levels of the dependent variable. As a result, the standard errors of regression coefficients are unreliable and may be understated.
...
<p style="border:3px; border-style:solid; border-color:#2B547E; padding: 1em;">
$H_o$: $\rho$ = 0, $\alpha$ = 0.05 
i.e., actual and predicted values of the dependent variable, time.years, are statistically independent.  
$H_a$: $\rho \ne$ 0   
i.e., actual and predicted values of the dependent variable, time.years, are statistically correlated.  
</p>
```{r,  fig.width=5, fig.height=5}
...
```
<div class = "blue">
From the analysis above, it can be inferred that actual and predicted values of the dependent variable, Life Expectancy, are significantly correlated (r = 0.3494662, p = 0.00002843). Since p < 0.05, the null hypotheses can be rejected. Also, the confidence interval does not include 0.
...
## Conclusion
<div class ="blue">
While our model did not have a perfect fit, it provided important information that shows having 4 or more lymph nodes, knowing the extent of the local spread, and the primary location of the tumor have importance on life expectancy in years.
</div>
# Survival analysis
...
legend("topright", legend=c("Probability of survival: Lev", "Probability of survival: Lev + 5FU"),fill=c("red","green"),bty="n")
...
* The survival probability of the Lev group is lower than the Lev+FU.
...
<div class = "blue">
  We cleaned the data from the study to suit our objectives and assessments.  With regards to the demographic distribution, there is an approximately equal distribution of males and females in the study. The study population is primarily in the 60+ age group followed by the 40-59 year subset. A very small proportion belongs to <40 years age group.  We considered two separate treatment groups for the analysis and both groups had an approximately equal representation.  
The following colon cancer variables were reviewed as part of the visualization: 

* Perforation: Almost all tumors did not perforate the colon.  
* Obstruction: Majority of tumors did not obstruct the colon.  
* Adherence; Majority of tumors did not adhere to nearby organs.  
* Differentiation: Majority of tumors had a histological differentiation of moderate.  
* Extent: Majority of the extent of localized tumor spread was to the serosa (lining) of the colon.  
* 5 Year Mortality Rate: Slightly more subjects were alive at the five year mark.  
* Lymph node Affected: Majority of the tumors did not effect more than four lymph nodes.  

The logistic regression model built to assess mortality within five years did not have a good fit, however it pointed out that factors such as age group, number of lymph nodes effected, and the extent of the spread have an impact on mortality within five years.

Stepwise regression create a model that also had a difficult fit due to variation in residuals, however it also pointed out that having more than four lymph nodes effected, the extent of the tumor spread, and the location of the primary tumor had an importance on time until death - treatment did not.

Survival analysis indicated that probability of survival was highest in the first 100 days regardless of treatment.  However, the adjuvant therapy of Levimasole and Fluorouracil provided a longer time for survival.

***In summary, it can can be said that while adjuvant therapy promotes a longer time until death, factors such as lymph node involvement, and extent of tumor spread have a higher impact on mortality in patients with Type B or C colon cancer.***
</div>
--- 
&nbsp;
<hr>
<p style="text-align: center;">Work done by Neha Kalra, Matthew Jabora, Ibrahim Zifa, Sandeep Delar, Romuald Lamps</p>
<p style="text-align: center;">eH 705 - M.Sc. eHealth</p>
