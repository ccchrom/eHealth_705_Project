...
The code-chunk below sets up the null model as "mort_5yr ~ 1" and shows its "Null deviance" of 750.68, which measures the variation among values of the response variable. That deviance becomes a basis of comparison for all other models. If a model can't significantly beat the Null model by producing a lower deviance, it is not worth pursuing.  
...
The Extent of local spread, and less significantly the age group and "more than 4 positive lymph nodes" seem to be the predictors of the 5 year life status. 

```{r}
#  + surg.factor + node4.factor + loccomp.factor
model_reg_all <- glm(mort_5yr ~ rx.factor + sex.factor + age + age.factor + obstruct.factor + perfor.factor + adhere.factor + differ.factor + extent.factor + surg.factor + nodes + node4 + loccomp.factor, family= binomial(logit), data = colon.data)
summary(model_reg_all)
```

## Use the Extent of local spread, the age factor and "more than 4 positive lymph nodes" variable

**TODO** Interpretation of the result

```{r}
model_reg_extent_age_node4 <- glm(mort_5yr ~ extent.factor + age.factor + node4, family= binomial(logit), data = colon.data)
...
But none are particularly efficient at determining the mortality at 5 years. The 'best' $R^2$ indicates that only 14.8% of the mortality at 5 years can be explained by the extent of the spread, the age group and "more than 4 nodes" factor.
...
The ANOVA test indicates that the models deviance are significantly different ( p = 0.00000000000000022 < 0.05)
...
Only the extent of the spread, the age factor and "more than 4 nodes" will be used in the prediction of the mortality at 5 years.
...
```{r}
training_model <- glm(mort_5yr ~ extent.factor + age.factor + node4, family= binomial(logit), data = train)
...
predicted.classes <- fct_relevel( (predicted.classes) , c("Alive","Died" ) )
str(predicted.prob)
...
```{r}
library(pROC)
test_model <- glm(mort_5yr ~ extent.factor + age.factor + node4, family= binomial(logit), data = test)
...
actual.num <- ifelse( test$mort_5yr == "Alive", 0, 1)
...
res.act.pred <- as.data.frame(cbind(actual.num, predicted.prob, residuals.num, test[c(14, 22, 26, 30)]) )
...
The actual and predicted values of Retplasma_2HiLo and the residuals.
