...
2.  To confirm using our own process that adjuvant therapy with the combination of levamisole and fluorouracil significantly reduce death within 5 years compared with no adjuvant therapy – using only levamisole.  

We aim to achieve this by:<br>
1.  Cleaning the data from the original and filtering it to our testing needs
2.  Using distribution plots and tables to assess the distributions of variables within the two treatment categories. 
...
6.  Using correlation and Variance Inflaction Factors (VIF) to investigate if any of the tumor descriptors or demographics have an impact on each other. (correlation)(VIF)  
...
<div class = "blue">
Every selected predictor appear significant (p-value < 0.05), indeed no odd ratio confidence interval include 1.
</div> 
...
```
<div class = "blue">
Using the stepAIC() function, it appears that the adjuvant therapy, age, differentiation, extent of spread, location of primary tumor have the greatest impact when assessing AIC.
</div>
...
<div class = "blue">
The RegModel.2 model above (model to the right), obtained through the stepwise regression (or stepwise selection) has the 'rx','age', 'differ','extent','node4', 'loccomp' have coefficients that are significantly different (p<0.05) from zero as shown by the p-values (p(sig)).Hence, the null hypotheses, which assume no relationships between the variables and the dependent variable ('time.years:'years until death'), for these variables would be rejected. 

Moreover, The adjusted R2(0.149), coefficient of multiple determination, is slightly higher in RegModel.2 model  with 6 predictor variables while the AIC, for the same model, is smaller (3735.011), i.e., better.
...
The second block gives the coefficients when 1 predictor is entered, 2 are entered through to all 6 predictors. lmg metric assesses the relative importance of each of the predictors to the relationship (between the dependent variable and the
predictor variables) both in terms of explanation and for prediction.
...
## Reporting The Training Residuals  
...
```
<div class ="blue">
From the Shapiro-Wilk normality test, we have got the p value of 0.04357. Assuming an alpha of 0.05, since the p-value is lesser than 0.05, null hypothesis (that distribution of residuals is Normal) is rejected. Hence it may be concluded that the residuals __are not normally distributed___

Hence it can be concluded that the regression model (model.tr) with variables 'rx','age','differ','extent','loccomp','node4' cannot explain/ predict that the trends in the dependent variable(time.years )
...
The Breush-Pagan test creates a statistic that is chi-squared distributed and for this data that statistic = 4.862106 .
The p-value is the result of the chi-squared test and the null hypothesis is rejected for p-value < 0.05. With the p value of 0.027 (<0.05), __the null hypothesis__ of homoskedasticity would be __rejected__.We should conclude that variance
...
<br>
  The logistic regression model built to assess mortality within five years did not have a good fit, however it pointed out that factors such as age group, number of lymph nodes effected, and the extent of the spread have an impact on mortaility within five years.
  Stepwise regression create a model that also had a difficult fit due to residuals, however it also pointed out that having more than four lymph nodes effected, the extent of the tumor spread, and the location of the primary tumor had an importance on time until death - treatment did not.
  Survival analysis indicated that probabilty of survival was highest in hte first 100 days regardless of treatment.  However, the adjuvant therapy of Levimasole and Fluorouracil provided a longer time for survival.
  <br>
  In summary, can can be said that while adjuvant therapy promotes a longer time until death, factors such as lymph node involvement, and extent of tumor spread have a higher impact on mortality.
